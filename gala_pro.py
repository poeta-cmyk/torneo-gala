...
st.markdown("""
    ...
    .alerta-roja-fija {
        background-color: #ff0000; color: white; padding: 20px; text-align: center;
        font-weight: 900; font-size: 30px; border: 4px solid black; margin-bottom: 20px;
...
with st.sidebar:
    ...
    if col1.button("▶️ Iniciar"): st.session_state.end_time = time.time() + minutos_ajuste * 60
...
# AQUÍ SE DEFINEN LAS PESTAÑAS (Esto es lo que faltaba)
...
with t1:
    ...
    def fila_res(n_m, ids):
        ...
        for p in st.session_state.db.get("puntos", []):
            if p["r"] == r_sel and p["ids_m"][0] == ids[0]:
                p_m_prev, p_v_prev = p["p_m"], p["p_v"]
    ...
    if st.button("💾 Guardar Ronda " + str(r_sel), use_container_width=True):
        ...
        with open(FILE, "w") as f: json.dump(st.session_state.db, f)
        st.balloons()
        st.success(f"¡Resultados de la Ronda {r_sel} guardados con éxito!")
